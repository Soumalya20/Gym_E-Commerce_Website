...
[Features](#-features) • [Quick Start](#-quick-start-guide) • [Installation](#-installation) • [Documentation](#-documentation) • [Support](#-support)
...
| **🛍️ Product Browsing** | Browse products with advanced filtering (category, price, search) |
...
| **💳 Checkout Flow** | Complete shipping address collection and payment processing |
| **💸 Payment Gateway** | Secure Razorpay integration with order verification |
| **📋 Order History** | View past orders and track delivery status |
| **⭐ Product Reviews** | Submit and view customer reviews with star ratings |

### 👨‍💼 Admin Features

| Feature | Description |
|---------|-------------|
| **📊 Dashboard** | Centralized control panel for store operations |
| **📦 Product Management** | Create, update, and delete products with full CRUD operations |
...
| `MONGO_URI` | MongoDB connection string | `mongodb://localhost:27017/koushiks-supplements` |
...
| `RAZORPAY_KEY_SECRET` | Razorpay API Key Secret | `your_key_secret_here` |
...
|--------|----------|-------------|---------------|------------|
| `GET` | `/products` | Get all products (with filters) | No | No |
| `GET` | `/products/:id` | Get single product | No | No |
| `POST` | `/products` | Create product | Yes | Yes |
| `PUT` | `/products/:id` | Update product | Yes | Yes |
| `DELETE` | `/products/:id` | Delete product | Yes | Yes |
| `POST` | `/products/:id/reviews` | Submit review | Yes | No |
...
| Method | Endpoint | Description | Auth Required | Admin Only |
|--------|----------|-------------|---------------|------------|
| `GET` | `/orders/mine` | Get user's orders | Yes | No |
| `GET` | `/orders` | Get all orders | Yes | Yes |
| `GET` | `/orders/:id` | Get order details | Yes | No* |
| `PUT` | `/orders/:id/deliver` | Mark as delivered | Yes | Yes |
...
| Method | Endpoint | Description | Auth Required |
|--------|----------|-------------|---------------|
| `POST` | `/payment/orders` | Create Razorpay order | Yes |
...
4. Whitelist IP (use `0.0.0.0/0` for all IPs in development)
...
5. Review [TESTING_REPORT.md](./TESTING_REPORT.md) for known issues
...
| [QUICK_START.md](./QUICK_START.md) | Quick setup guide for experienced developers |
| [RAZORPAY_SETUP.md](./RAZORPAY_SETUP.md) | Detailed Razorpay payment integration guide |
| [TESTING_REPORT.md](./TESTING_REPORT.md) | Comprehensive testing results and checklist |
| [DEPLOYMENT.md](./DEPLOYMENT.md) | Step-by-step deployment instructions |
| [IMPROVEMENTS_SUMMARY.md](./IMPROVEMENTS_SUMMARY.md) | Recent improvements and fixes |
...
2. **Common Questions**
   ...
   - **Q: Where do I get Razorpay keys?**
     - A: See [RAZORPAY_SETUP.md](./RAZORPAY_SETUP.md)
...
[⬆ Back to Top](#-koushiks-supplements-e-commerce-platform)
